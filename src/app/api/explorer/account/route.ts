...
export async function GET(req: Request) {
    try {
        ...
        const action = searchParams.get('action'); // balance | txlist | txlistinternal | tokentx
        const sort = searchParams.get('sort'); // asc | desc (optional)
        const contractaddress = searchParams.get('contractaddress'); // for tokentx
        ...
        const allowedActions = new Set(['balance', 'txlist', 'txlistinternal', 'tokentx']);
        ...
        let base = `https://api.etherscan.io/v2/api?chainid=8453&module=account&action=${encodeURIComponent(action)}&address=${encodeURIComponent(address)}`;

        // Add contractaddress for tokentx
        if (action === 'tokentx' && contractaddress) {
            base += `&contractaddress=${encodeURIComponent(contractaddress)}`;
        }
