...
const getContractAddress = async () => {
  ...
  const addr = await contractAddressPromise;
  if (!addr || addr === '') {
    contractAddressPromise = getDiamondAddress();
    return await contractAddressPromise;
  }
  return addr;
};

// Role Definition - TLN Specific
const CHIEF_OF_POLICE_ROLE = 'Grand Loch Strategist';

// Emerald color palette for charts
const COLORS = ['#00bf8a', '#34d399', '#6ee7b7', '#a7f3d0', '#d1fae5', '#ffffff', '#004d36', '#00755c'];
...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-black/90 border border-brand-500/30 p-3 rounded-lg shadow-xl text-xs">
        <p className="font-bold text-white mb-1">{timestamp}</p>
        <p className="text-white/80">Balance: {balance} ETH</p>
        {typeof percentageChange === 'number' && (
          <p className={percentageChange >= 0 ? 'text-brand-400' : 'text-red-400'}>
...
const Modal = ({ open, onClose, title, children }: { open: boolean, onClose: () => void, title: string, children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-[#001a12] border border-brand-500/30 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="flex justify-between items-center p-4 border-b border-brand-500/20 bg-brand-950/50">
          <h3 className="text-lg font-bold text-white tracking-wide font-rajdhani">{title}</h3>
          <button onClick={onClose} className="p-1 hover:bg-brand-500/20 rounded-full transition-colors">
...
const Reserve = () => {
  ...
  const fetchTransactionData = async () => {
    try {
      ...
      setTransactionCount(combinedTransactions.length);
      setLastTransactions(combinedTransactions.slice(-4).reverse());
      ...
      const tokenHistoryBuilder = async (tokenAddress: string) => {
        try {
          ...
          let decimals = tokenAddress === USDC_ADDRESS ? 6 : 8;
          let running = 0n;
          const points: any[] = [];
      ...
      try {
        ...
        const [usdcBalRaw, cbbtcBalRaw] = await Promise.all([
        ...
        const usdcBal = Number(usdcBalRaw) / Math.pow(10, 6);
        const cbbtcBal = Number(cbbtcBalRaw) / Math.pow(10, 8);
  ...
  const processBalanceHistory = async (transactions: any[]) => {
    try {
      ...
      sortedTransactions.forEach((txn) => {
        const value = BigInt(txn.value || '0');
        ...
        if (previousBalance !== 0n) {
          percentageChange = Number(balance - previousBalance) / Number(previousBalance) * 100;
        }

        balanceHistoryData.push({
          timestamp: new Date(timestamp).toLocaleDateString(),
          balance: Number(formatEther(balance.toString())),
          percentageChange,
        });
    ...
    return (
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          ...
          <Tooltip contentStyle={{ backgroundColor: '#001a12', border: '1px solid rgba(0,191,138,0.3)' }} />
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-black text-brand-400 font-mono animate-pulse">
  ...
  return (
    <div className="min-h-[96vh] p-4 md:p-8 text-white overflow-y-auto font-sans relative">
      ...
      <div className="max-w-7xl mx-auto space-y-8 mt-4 pb-20">
        {/* Title Section */}
        <div className="flex flex-col md:flex-row justify-between items-center bg-[#001a12]/80 backdrop-blur-xl p-6 rounded-2xl border border-brand-500/30 shadow-[0_0_30px_rgba(0,191,138,0.15),inset_0_1px_0_rgba(255,255,255,0.05)]">
          <div>
            <h1 className="text-3xl font-bold tracking-tight mb-1 text-transparent bg-clip-text bg-gradient-to-r from-brand-300 to-white font-rajdhani">
              Reserve Dashboard
            </h1>
            <p className="text-brand-400/50 text-sm font-mono tracking-widest">
              LOCH NESS SOCIETY TREASURY
            </p>
          </div>
          <button onClick={manualRefresh} className="p-2 hover:bg-brand-500/20 rounded-full transition-colors" title="Refresh Data">
            <ArrowPathIcon className={`w-6 h-6 text-brand-400 ${loading ? 'animate-spin' : ''}`} />
          </button>
          ...
          {isChief && (
            <div className="flex gap-2 mt-4 md:mt-0">
              <button
                ...
                className="flex items-center gap-2 bg-brand-500/20 hover:bg-brand-500/30 text-brand-300 border border-brand-500/30 rounded-full px-4 py-2 transition-all"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* ETH Balance */}
          <div className="bg-[#001a12]/80 backdrop-blur-xl border border-brand-500/30 p-6 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden group shadow-[0_0_25px_rgba(0,191,138,0.1),inset_0_1px_0_rgba(255,255,255,0.05)]">
            <div className="absolute inset-0 bg-gradient-to-br from-brand-500/10 to-transparent group-hover:from-brand-400/20 transition-all duration-300"></div>
            <h2 className="text-brand-300/70 font-mono text-sm relative z-10 tracking-wider">ETH RESERVE</h2>
            <p className="text-4xl font-bold mt-2 relative z-10 text-white drop-shadow-[0_0_10px_rgba(0,191,138,0.3)]">{balance} Ξ</p>
            <p className="text-brand-300/50 text-xs mt-1 relative z-10">
          ...
          <div className="bg-[#001a12]/80 backdrop-blur-xl border border-brand-500/30 p-6 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden group shadow-[0_0_25px_rgba(0,191,138,0.1),inset_0_1px_0_rgba(255,255,255,0.05)]">
            <div className="absolute inset-0 bg-gradient-to-br from-brand-500/10 to-transparent group-hover:from-brand-400/20 transition-all duration-300"></div>
            <h2 className="text-brand-300/70 font-mono text-sm relative z-10 tracking-wider">USDC RESERVE</h2>
            <p className="text-4xl font-bold mt-2 relative z-10 text-white drop-shadow-[0_0_10px_rgba(0,191,138,0.3)]">{usdcBalance}</p>
            <p className="text-brand-300/50 text-xs mt-1 relative z-10">
          ...
          <div className="bg-[#001a12]/80 backdrop-blur-xl border border-brand-500/30 p-6 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden group shadow-[0_0_25px_rgba(0,191,138,0.1),inset_0_1px_0_rgba(255,255,255,0.05)]">
            <div className="absolute inset-0 bg-gradient-to-br from-brand-500/10 to-transparent group-hover:from-brand-400/20 transition-all duration-300"></div>
            <h2 className="text-brand-300/70 font-mono text-sm relative z-10 tracking-wider">BTC RESERVE</h2>
            <p className="text-4xl font-bold mt-2 relative z-10 text-white drop-shadow-[0_0_10px_rgba(0,191,138,0.3)]">{cbbtcBalance}</p>
            <p className="text-brand-300/50 text-xs mt-1 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Balance History */}
          <div className="bg-[#001a12]/80 backdrop-blur-xl border border-brand-500/30 rounded-2xl p-6 min-h-[400px] shadow-[0_0_25px_rgba(0,191,138,0.1),inset_0_1px_0_rgba(255,255,255,0.05)]">
            <div className="flex items-center gap-2 mb-6">
              <ChartBarIcon className="w-5 h-5 text-brand-400" />
              <h3 className="text-lg font-bold text-white font-rajdhani">Reserves History</h3>
            </div>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={balanceHistory}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,191,138,0.1)" />
                  <XAxis dataKey="timestamp" stroke="#00bf8a" fontSize={12} tickFormatter={(t) => t.split('/')[0] + '/' + t.split('/')[1]} />
                  <YAxis stroke="#00bf8a" fontSize={12} />
                  <Tooltip content={<CustomTooltip />} />
                  <Legend wrapperStyle={{ color: '#fff' }} />
                  <Line type="monotone" dataKey="balance" stroke="#fff" strokeWidth={2} dot={false} activeDot={{ r: 6 }} name="ETH" />
                  <Line type="monotone" dataKey="usdc" stroke="#34d399" strokeWidth={2} dot={false} name="USDC" />
                  <Line type="monotone" dataKey="cbbtc" stroke="#6ee7b7" strokeWidth={2} dot={false} name="BTC" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-[#001a12]/80 backdrop-blur-xl border border-brand-500/30 rounded-2xl p-6 min-h-[400px] shadow-[0_0_25px_rgba(0,191,138,0.1),inset_0_1px_0_rgba(255,255,255,0.05)]">
            <div className="flex items-center gap-2 mb-6">
              <ChartBarIcon className="w-5 h-5 text-brand-400" />
              <h3 className="text-lg font-bold text-white font-rajdhani">Allocation Distribution</h3>
            </div>
        ...
        {/* Beneficiaries List */}
        <div className="bg-[#001a12]/80 backdrop-blur-xl border border-brand-500/30 rounded-2xl p-6 overflow-hidden shadow-[0_0_25px_rgba(0,191,138,0.1),inset_0_1px_0_rgba(255,255,255,0.05)]">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-white font-rajdhani">Authorized Beneficiaries</h3>
            {isChief && (
              <button
                onClick={() => setOpenAddBeneficiary(true)}
                className="bg-brand-500/20 hover:bg-brand-500/30 text-brand-300 rounded-full p-2 transition-colors border border-brand-500/30"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-brand-500/20 text-brand-400/50 text-sm font-mono">
              ...
              <tbody>
                {beneficiaries.map((b, i) => (
                  <tr key={i} className="border-b border-brand-500/10 hover:bg-brand-500/5 transition-colors">
                    ...
                    <td className="p-3 text-right text-brand-400">{Number(b.split)}%</td>
                    {isChief && (
                      <td className="p-3 text-right flex justify-end gap-2">
                        <button
                          ...
                          className="p-1 hover:text-brand-400 text-gray-400 transition-colors"
                        >
      ...
      <Modal open={openAddBeneficiary} onClose={() => setOpenAddBeneficiary(false)} title="Add Beneficiary">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-mono text-brand-400/60 mb-1">WALLET ADDRESS</label>
            <input
              type="text"
              value={newBeneficiary.address}
              onChange={(e) => setNewBeneficiary({ ...newBeneficiary, address: e.target.value })}
              className="w-full bg-black/50 border border-brand-500/30 rounded-lg p-3 text-white focus:border-brand-400 outline-none transition-colors"
              placeholder="0x..."
            />
          </div>
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="block text-xs font-mono text-brand-400/60 mb-1">SPLIT %</label>
              <input
                type="number"
            ...
            <div className="flex-1">
              <label className="block text-xs font-mono text-brand-400/60 mb-1">ROLE</label>
              <input
                type="text"
          ...
          <button
            ...
            className="w-full bg-brand-500 text-black font-bold py-3 rounded-lg hover:bg-brand-400 transition-colors mt-2"
          >
            Confirm Add
          </button>
        </div>
      ...
      <Modal open={openWriteFunctionDialog} onClose={() => setOpenWriteFunctionDialog(false)} title={selectedFunction}>
        <div className="space-y-4">
          {selectedFunction === 'depositFunds' && (
            <>
              <div>
                <label className="block text-xs font-mono text-brand-400/60 mb-1">AMOUNT (ETH)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-mono text-brand-400/60 mb-1">NOTE</label>
                <input
                  type="text"
          ...
          {selectedFunction === 'withdraw' && (
            <p className="text-brand-400/60 text-sm">This will withdraw funds according to the beneficiary splits.</p>
          )}
          {selectedFunction === 'sendFundsToUtilityCoDiamond' && (
            <>
              <div>
                <label className="block text-xs font-mono text-brand-400/60 mb-1">PERCENTAGE</label>
                <input
                  type="number"
          ...
          {selectedFunction === 'wipeBeneficiaries' && (
            <p className="text-red-400 text-sm">⚠️ This will remove ALL beneficiaries. This action cannot be undone.</p>
          )}
          <button
