...
const navigationDots = [
  { name: 'Discord', href: 'https://discord.gg/q4tFymyAnx', color: 'bg-emerald-800' },
  { name: 'Whitepaper', href: 'https://docs.thelochnessbotanicalsociety.com', color: 'bg-emerald-600' },
  { name: 'About Us', href: 'https://www.thelochnessbotanicalsociety.com', color: 'bg-emerald-400' },
];
...
export default function SelectionBar({ onPanelChange, currentPanel }: SelectionBarProps) {
  ...
  return (
    <div className="relative w-full z-50">
      {/* Selection Bar */}
      <div
        className="relative flex justify-center items-center py-0 px-6 w-full backdrop-filter backdrop-blur-md bg-emerald-900 bg-opacity-30 shadow-lg rounded-lg"
        style={{
          ...
          <>
              ...
              {panelsLeft.map((panel) => (
                <button
                  ...
                  className={`transition-all duration-300 ${
                    panel === currentPanel
                      ? 'text-white text-base bg-emerald-700 py-1 px-2 rounded-md'
                      : 'text-emerald-100 text-sm py-1 px-2'
                  }`}
                  style={
                    panel === currentPanel
            ...
            <div className="relative flex justify-center items-center mx-4" style={{ zIndex: 30 }}>
              <button
                ...
                className="relative p-0 hover:ring-4 hover:ring-emerald-500 rounded-full transition-all duration-300 transform hover:scale-105"
                style={{
                  top: '-10px', // Move the medallion up a bit
                  zIndex: 30, // Higher z-index to stay above the pop-up menu
                }}
              >
                <img src="/Medallions/TLN.png" alt="Medallion" className="h-14 w-14 rounded-full" />
              </button>
            </div>
      ...
      {menuOpen && ( // Conditionally render the menu only when open
        <div
          className={`absolute left-1/2 transform -translate-x-1/2 bottom-20 z-10 transition-all duration-500 ease-in-out opacity-100 flex flex-col items-center space-y-4 backdrop-filter backdrop-blur-md bg-emerald-900 bg-opacity-30 p-4 rounded-lg shadow-lg`}
          style={{
            boxShadow: '0px 4px 12px rgba(0, 0, 0, 0.2)',
            backdropFilter: 'blur(10px)',
            WebkitBackdropFilter: 'blur(10px)',
