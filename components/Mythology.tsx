...
export default function Mythology() {
  ...
  const checkPublisherRole = async () => {
    ...
    // Guard against missing address or account
    ...
    try {
      ...
      // TLN uses lnHasRole instead of ieHasRole
      const hasRole = await readContract({
        contract,
        method: 'lnHasRole',
        params: ["LN Default Admin Role", account?.address as string]
      });

      setIsPublisher(hasRole as boolean);
    } catch (e) {
      // Silently handle if method not available or other error
      console.debug("Role check failed (non-critical):", e);
      setIsPublisher(false);
    }
  }
  ...
  if (loading && stories.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full w-full relative overflow-hidden font-rajdhani">
        <div className="bg-[#001a12]/80 backdrop-blur-xl p-8 rounded-2xl border border-brand-500/30 shadow-[0_0_30px_rgba(0,191,138,0.15)]">
          <div className="bg-brand-500/10 p-6 rounded-full animate-ping mb-4 mx-auto w-fit">
            <GlobeAltIcon className="w-12 h-12 text-brand-400" />
          </div>
          <div className="text-brand-300 font-bold tracking-[0.3em] text-sm animate-pulse text-center">
  ...
  return (
    <div className="relative flex flex-col lg:flex-row h-full w-full text-white overflow-hidden font-rajdhani">
      ...
      <div
        className={`flex-shrink-0 border-r border-brand-500/30 bg-[#001a12]/80 backdrop-blur-xl flex flex-col z-40 h-full transition-all duration-300 ease-in-out absolute lg:relative shadow-[0_0_20px_rgba(0,191,138,0.1)]
          ${isNavOpen ? 'translate-x-0 w-full lg:w-80' : '-translate-x-full lg:translate-x-0 lg:w-0 lg:border-none lg:overflow-hidden'}
        `}
      >
        <div className="p-4 lg:p-6 border-b border-brand-500/20 relative overflow-hidden shrink-0 flex justify-between items-center">
          ...
          <h1 className="text-xl lg:text-2xl font-bold tracking-tight text-white flex items-center gap-2 relative z-10">
            ...
            LORE<span className="text-brand-400">//</span>DB
          </h1>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 min-h-0">
          <div className="text-xs font-bold text-brand-300 uppercase tracking-wider mb-2 px-4 mt-4 flex items-center gap-2">
              ...
              <div key={story.id} className="group">
                <button
                  ...
                  className={`w-full text-left px-3 py-2 lg:px-4 lg:py-3 rounded-lg transition-all duration-300 border border-transparent backdrop-blur-sm relative overflow-hidden ${selectedStory?.id === story.id
                    ? 'bg-brand-500/10 border-brand-500/50 text-white shadow-[0_0_15px_rgba(0,191,138,0.15)]'
                    : 'hover:bg-brand-500/5 hover:border-brand-500/10 text-gray-400'
                    }`}
                >
                ...
                {selectedStory?.id === story.id && (
                  <div className="mt-2 ml-4 pl-4 border-l border-brand-500/20 space-y-1">
                    {chapters.map(chapter => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-xs rounded-md transition-all duration-200 flex items-center gap-2 group/chapter ${selectedChapter?.id === chapter.id
                          ? 'text-white bg-brand-500/10 font-bold'
                          : 'text-gray-500 hover:text-gray-300 hover:bg-brand-500/5'
                          }`}
                      >
                        <ChevronRightIcon className={`w-3 h-3 transition-transform ${selectedChapter?.id === chapter.id ? 'rotate-90 text-brand-400' : 'opacity-0 group-hover/chapter:opacity-50'}`} />
        ...
        {/* Footer Actions */}
        <div className="p-4 border-t border-brand-500/20 bg-black/80 shrink-0">
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 px-4 py-3 border rounded-md text-xs font-bold transition-all duration-300 group ${isPublisher
              ? `cursor-pointer ${isWorkshopOpen ? 'bg-brand-500/20 border-brand-400 text-brand-300' : 'border-brand-500 text-brand-400 hover:bg-brand-500 hover:text-black'}`
              : 'border-dashed border-brand-500/20 text-gray-500 cursor-not-allowed hover:bg-brand-500/5'
              }`}
          >
            {isPublisher ? (
              <>
              ...
              <>
                <DocumentPlusIcon className="w-4 h-4 group-hover:text-brand-400 transition-colors" />
      ...
      <div className="flex-1 flex flex-col relative z-0 h-full overflow-hidden bg-black/20 border-y border-brand-500/10">

        {/* Mobile Nav Toggle Overlay Button (When Nav Closed) */}
        {!isNavOpen && (
          <button
            onClick={() => setIsNavOpen(true)}
            className="absolute top-4 left-4 z-50 p-2 bg-black/50 backdrop-blur border border-brand-500/20 rounded-full text-brand-400 shadow-lg hover:bg-black/80 transition-all"
          >
        ...
        {isWorkshopOpen ? (
          ...
          <div className="fixed inset-0 z-50 lg:relative lg:z-auto lg:h-full flex flex-col bg-black/95 lg:bg-black/80 backdrop-blur-xl">
            ...
            <div className="flex flex-col border-b border-brand-500/20 bg-black/80 backdrop-blur-xl z-20 shrink-0">
              {/* Top Bar: Title & Tabs */}
              <div className="p-4 flex items-center justify-between border-b border-brand-500/10">
                <div className="flex items-center gap-4">
                  <CommandLineIcon className="w-5 h-5 text-brand-400" />
                  <h2 className="text-xs font-bold tracking-widest text-white uppercase">
                    Workshop <span className="text-brand-400">//</span> {workshopMode}
                  </h2>
                </div>
                {/* Mode Selector Tabs */}
                  ...
                  {(['STORIES', 'CHAPTERS', 'SECTIONS'] as const).map(mode => (
                    <button
                      ...
                      className={`px-3 py-1 text-[10px] font-bold border rounded transition-all ${workshopMode === mode
                        ...
                        : 'border-brand-500/20 text-gray-400 hover:border-brand-500/50 hover:text-brand-300'
                        }`}
                    >
                      {mode}
                    </button>
              ...
              {/* Context Bar */}
              <div className="px-4 py-3 flex items-center gap-4 text-[10px] text-gray-400 bg-black/40 overflow-x-auto whitespace-nowrap border-b border-brand-500/10">
                <div className="flex items-center gap-2">
                  <BookOpenIcon className="w-3 h-3 text-brand-400" />
                  ...
                  <select
                    ...
                    className="bg-brand-950/50 border border-brand-500/20 rounded px-2 py-1 text-white focus:border-brand-400 outline-none min-w-[150px]"
                  >
                ...
                <div className="flex items-center gap-2">
                  <ListBulletIcon className="w-3 h-3 text-brand-400" />
                  ...
                  <select
                    ...
                    className="bg-brand-950/50 border border-brand-500/20 rounded px-2 py-1 text-white focus:border-brand-400 outline-none min-w-[150px] disabled:opacity-30"
                  >
                ...
                {workshopMode === 'SECTIONS' && (
                  <>
                    <ChevronRightIcon className="w-3 h-3 opacity-30" />
                    <div className="flex items-center gap-2">
                      <PencilSquareIcon className="w-3 h-3 text-brand-400" />
              ...
              {/* Toolbar (Only for SECTIONS mode) */}
              {workshopMode === 'SECTIONS' && (
                <div className="px-4 py-2 flex items-center gap-1 bg-black/60 border-b border-brand-500/10">
                  {[
                    ...
                    <button
                      ...
                      className="w-7 h-7 flex items-center justify-center rounded hover:bg-brand-500/10 text-gray-400 hover:text-brand-300 transition-colors"
                    >
                  ...
                  <button
                    ...
                    className={`px-2 h-7 flex items-center gap-2 rounded transition-colors text-[10px] ${showMetadataPanel ? 'bg-brand-500/20 text-brand-300' : 'hover:bg-brand-500/10 text-gray-400'}`}
                  >
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar relative">
                ...
                {workshopMode === 'STORIES' && (
                  <div className="p-8 md:p-12 max-w-3xl mx-auto space-y-8">
                    <div className="bg-brand-950/30 border border-brand-500/20 rounded-lg p-6">
                      <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                        <SparklesIcon className="w-4 h-4 text-brand-400" /> CREATE NEW ARCHIVE (STORY)
                      </h3>
                      <div className="space-y-4">
                        <input
                          ...
                          className="w-full bg-black/30 border border-brand-500/20 rounded px-4 py-3 text-white focus:border-brand-400 outline-none"
                        />
                        <button onClick={handleWorkshopSubmit} disabled={isTxPending || !formData.title} className="w-full py-3 bg-brand-500 hover:bg-brand-400 text-black font-bold rounded">
                    ...
                    <div className="space-y-2">
                      <label className="text-xs text-gray-500 uppercase">Existing Archives</label>
                      {stories.map(s => (
                        <div key={s.id} className="p-4 border border-brand-500/10 bg-brand-950/20 rounded flex justify-between items-center text-sm text-gray-300">
                          <span>{s.title}</span>
                  ...
                  <div className="p-8 md:p-12 max-w-3xl mx-auto space-y-8">
                    {!selectedStory ? (
                      ...
                      <>
                        <div className="bg-brand-950/30 border border-brand-500/20 rounded-lg p-6">
                          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                            <SparklesIcon className="w-4 h-4 text-brand-400" /> ADD FRAGMENT TO: {selectedStory.title}
                          </h3>
                          <div className="space-y-4">
                            <input
                              type="text"
                        ...
                        <div className="space-y-2">
                          <label className="text-xs text-gray-500 uppercase">Existing Fragments</label>
                          {chapters.map(c => (
                            <div key={c.id} className="p-4 border border-brand-500/10 bg-brand-950/20 rounded flex justify-between items-center text-sm text-gray-300">
                              <span>{c.title}</span>
                              <span className="text-xs opacity-50">#{c.id}</span>
                            </div>
                          ))}
                        </div>
                      </>
                ...
                {workshopMode === 'SECTIONS' && (
                  <div className="p-8 md:p-16 max-w-3xl mx-auto space-y-6">
                    {(selectedStory && selectedChapter) ? (
                      <>
                        {formData.mediaURI && (
                          <div className="relative w-full bg-[#001a12] border-b border-brand-500/20 overflow-hidden mb-12 select-none rounded-b-[2rem] shadow-2xl">
                            <div className="absolute top-0 left-0 right-0 h-24 bg-gradient-to-b from-brand-500/10 to-transparent z-10 pointer-events-none" />
                            <img
                        ...
                        <input
                          ...
                          type="text"
                          className="w-full bg-transparent border-none p-0 text-4xl md:text-5xl font-black text-white focus:outline-none focus:ring-0 placeholder:text-brand-500/20 tracking-tight leading-tight"
                        ...
                        <textarea
                          ...
                          className="w-full h-[calc(100vh-400px)] bg-transparent border-none p-0 text-lg text-gray-300 font-serif leading-8 focus:outline-none focus:ring-0 placeholder:text-brand-500/20 resize-none"
                        ...
                        <div className="pt-8 flex justify-end">
                          <button
                            ...
                            className="px-6 py-3 bg-brand-500 hover:bg-brand-400 text-black text-xs font-bold tracking-widest rounded transition-all shadow-[0_0_20px_rgba(0,191,138,0.3)] hover:shadow-[0_0_40px_rgba(0,191,138,0.5)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                          >
                            {isTxPending ? (
                              <>
              ...
              {showMetadataPanel && (
                <div className="w-80 bg-black/60 backdrop-blur-md border-l border-brand-500/20 p-6 overflow-y-auto shrink-0 animate-in slide-in-from-right duration-300">
                  <h3 className="text-xs font-bold tracking-widest text-white mb-6 flex items-center gap-2">
                    <TagIcon className="w-4 h-4 text-brand-400" /> METADATA_FIELDS
                  </h3>

                  <div className="space-y-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest text-brand-300 flex items-center gap-2">
                      ...
                      <input
                        ...
                        type="text"
                        className="w-full bg-brand-950/30 border border-brand-500/20 rounded px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-400 transition-colors"
                        placeholder="ipfs://..."
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full bg-brand-950/30 border border-brand-500/20 rounded px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-400 transition-colors resize-none h-20"
            ...
            <div className="max-w-4xl mx-auto w-full">
              ...
              {selectedSection.mediaURI && (
                <div className="relative w-full h-64 lg:h-96 overflow-hidden select-none shadow-2xl">
                  <div className="absolute inset-0 bg-gradient-to-t from-[#001a12] via-transparent to-transparent z-10" />
                  <img
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8 lg:p-12 z-20">
                    ...
                    <div className="flex flex-wrap gap-2 text-[10px] font-bold tracking-widest text-brand-300/60 uppercase">
                      <span className="bg-brand-950/50 px-2 py-1 rounded backdrop-blur">
                        ID: {selectedSection.id}
                      </span>
                      <span className="bg-brand-950/50 px-2 py-1 rounded backdrop-blur flex items-center gap-1">
              ...
              <div className="p-8 lg:p-16 relative">
                {!selectedSection.mediaURI && (
                  <div className="mb-12 border-b border-brand-500/20 pb-8">
                ...
                {/* Main Text Content */}
                <div className="prose prose-invert prose-lg max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-p:leading-relaxed prose-p:text-gray-300 prose-blockquote:border-l-2 prose-blockquote:border-brand-500/50 prose-blockquote:pl-4 prose-blockquote:italic prose-blockquote:text-gray-400">
                    ...
                    <React.Fragment key={i}>
                      {line.startsWith('# ') ? <h1 className="text-3xl font-black mt-8 mb-4">{line.replace('# ', '')}</h1> :
                        line.startsWith('## ') ? <h2 className="text-2xl font-bold mt-6 mb-3">{line.replace('## ', '')}</h2> :
                          line.startsWith('> ') ? <blockquote className="my-4 pl-4 border-l-2 border-brand-500/30 italic text-gray-400">{line.replace('> ', '')}</blockquote> :
                            line.trim() === '' ? <br /> :
                ...
                {(selectedSection.sources.length > 0 || selectedSection.keywords.length > 0) && (
                  <div className="mt-16 pt-8 border-t border-brand-500/20 grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedSection.keywords.map((k, i) => (
                            <span key={i} className="px-2 py-1 rounded bg-brand-950/30 border border-brand-500/10 text-[10px] font-medium text-gray-400">
                              {k}
                            </span>
                          ))}
                        </div>
                        ...
                        <ul className="space-y-1">
                          {selectedSection.sources.map((s, i) => (
                            <li key={i} className="text-xs text-gray-500 truncate flex items-center gap-2">
                              <span className="w-1 h-1 bg-brand-500/30 rounded-full" /> {s}
                            </li>
                          ))}
                        </ul>
                      </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-gray-600 relative overflow-hidden">
            ...
            <BookOpenIcon className="w-16 h-16 mb-4 opacity-20 text-brand-500" />
            <h3 className="text-sm font-bold tracking-[0.2em] uppercase mb-2 text-brand-400/50">Secure Archives Locked</h3>
